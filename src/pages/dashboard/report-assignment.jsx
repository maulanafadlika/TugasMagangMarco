...
const  ReportAssignment = ()=> {
  ...
  const {dataReport,fetchDataProjectAssignment,loading,fetchDataReportAssignment} = UseReportAssign()
  const PER_PAGE = 10;
  const offset = currentPage * PER_PAGE;
    ...
    useEffect(() => {
        const fetchProject = async () => {
            const result = await fetchDataProjectAssignment();
            if (result) {
    ...
    useEffect(() => {
        ...
        const fetchReport = async () => {
            const result = await fetchDataReportAssignment(formData.project_id);
            if(result.data){
  ...
  const sortedRows = useMemo(() => {
    ...
    return [...TABLE_ROWS].sort((a, b) => {
      ...
      let valueB = b[key];
        ...
        const dateFields = ['startdate', 'duedate', 'updated_time', 'resolved_time', 'created_time'];
  ...
  const tableRows = currentPageData.map((row, index) => {
    ...
    return (
      <tr key={uniqueKey} className="border-b border-gray-200 font-poppins text-xs" style={{ color: '#212529' }}>
        <td className="border border-gray-300 p-4">{highlightText(row.issue_type)}</td>
        <td className="border border-gray-300 p-4">{highlightText(row.issue_code)}</td>
        <td className="border border-gray-300 p-4">{highlightText(row.summary)}</td>
        <td className="border border-gray-300 max-w-[600px] p-4">{highlightText(row.description)}</td>
        <td className="border border-gray-300 p-4">{highlightText(row.assignee)}</td>
        <td className="border border-gray-300 p-4">{highlightText(row.assignee_id)}</td>
        <td className="border border-gray-300 p-4">{highlightText(row.created_by)}</td>
        <td className="border border-gray-300 p-4">{highlightText(row.created_id)}</td>
        <td className="border border-gray-300 p-4">
          {highlightText(row.startdate ? formatDate(row.startdate,true) : '')}
        </td>
        <td className="border border-gray-300 p-4">
          {highlightText(row.duedate ? formatDate(row.duedate,true) : '')}
        </td>
        <td className="border border-gray-300 p-4">
        ...
        <td className="border border-gray-300 p-4">
          {highlightText(row.created_time ? formatDate(row.created_time,true): '')}
        </td>
        <td className="border border-gray-300 p-4">
          {highlightText(row.updated_time ? formatDate(row.updated_time,true,true) : '')}
        </td>
        <td className="border border-gray-300 p-4">
          {highlightText(row.resolved_time ? formatDate(row.resolved_time,true) : '')}
        </td>
      </tr>
    );
  });  
  ...
  const headers = [
    ...
    { label: 'Issue Type', key: 'issue_type', width: 15 },
    { label: 'Issue Code', key: 'issue_code', width: 25 },
    { label: 'Summary', key: 'summary', width: 15 },
    { label: 'Description', key: 'description', width: 15 },
    { label: 'Assignee', key: 'assignee', width: 15 },
    { label: 'Assignee Id', key: 'assignee_id', width: 15 },
    { label: 'Created By', key: 'created_by', width: 15 },
    { label: 'Created Id', key: 'created_id', width: 17 },
    { label: 'Start Date', key: 'startdate', width: 20 },
    { label: 'Due Date', key: 'duedate', width: 20 },
    { label: 'Status', key: 'status', width: 15 },
    { label: 'Created Time', key: 'created_time', width: 15 },
    { label: 'Updated Time', key: 'updated_time', width: 15 },
    { label: 'Resolved Time', key: 'resolved_time', width: 15 },
  ];
  ...
  const LoadingOption = () => (
    ...
    const formatedTableRows = TABLE_ROWS.map(row => ({
        ...row,
        startdate: formatDate(row.startdate, true),
        duedate: formatDate(row.duedate, true),
        created_time: formatDate(row.created_time, true),
        updated_time: formatDate(row.updated_time, true,true),
        resolved_time: formatDate(row.resolved_time, true,true),
    }));
            ...
            {projectSelected && (
              <div className="flex items-center ml-4">
                ...
                <ExcelExportButton
                  ...
                  headers={headers}
                  customName={`${formData.project_id}_Report_Assignment`}
                />
              </div>
            )}
          </div>
        ...
        <CardBody className="overflow-scroll px-0">
          {formData.project_id ? (
            <table className="w-full min-w-max table-auto text-left font-poppins border border-gray-300">
              ...
              <tbody>
                {loading.fetchDataReportAssignLoading ? (
                  <tr>
